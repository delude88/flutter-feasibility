        ...
        switch (event.event) {
          ...
          case "candidate": {
            ...
            if (roomId) {
              ...
              if (typeof event.payload == "object" && data.to && data.from && data.candidate) {
                assert(groups[roomId] !== undefined)
                // Get target user from current group
                if (groups[roomId][data.to]) {
                  ws.send(createEvent('candidate', data))
                }
              }
            }
            break;
          ...
          case "description": {
            ...
            if (roomId) {
              ...
              if (typeof event.payload == "object" && data.to && data.from && data.description) {
                assert(groups[roomId] !== undefined)
                // Get target user from current group
                if (groups[roomId][data.to]) {
                  ws.send(createEvent('description', data))
                }
              }
            }
            break;
